...
def normalize_minecraft_versions(value, source):
    if not isinstance(value, list) or not value:
        raise ValueError(f"{source}: minecraft_versions must contain at least one version")
...
def load_static_mod(data, source):
    ...
    if isinstance(icon_url, str) and icon_url.strip():
        ...
        data["icon_url"] = f"{hub_raw_base}/mods/{data['id']}/icon.png"
    else:
    ...
    for release in releases:
        if not isinstance(release, dict):
            raise ValueError(f"{source}: releases must contain objects")
        version = require_text(release, "version", source)
        minecraft_versions = normalize_minecraft_versions(release.get("minecraft_versions"), source)
        ...
        published_at = require_text(release, "published_at", source)
        normalized = {
            "version": version,
    ...
    normalized_releases.sort(key=lambda release: release["published_at"], reverse=True)
...
def github_release_url(repository, per_page):
    return f"{github_api}/repos/{repository}/releases?per_page={per_page}"


def raw_url(repository, ref, path):
    return (
        f"https://raw.githubusercontent.com/{repository}/"
        f"{quote(ref, safe='')}/{quote(path.lstrip('/'), safe='/')}"
    )
...
def load_manifest(repository, metadata_path, ref):
    ...
    data = json.loads(request_text(raw_url(repository, ref, metadata_path)))
...
def normalize_asset_rules(manifest, source):
    ...
    if not isinstance(rules, dict):
        raise ValueError(f"{source}: release_assets must be an object")
    include = rules.get("include", ["*.levipack", "*.so", "*.zip"])
    exclude = rules.get("exclude", [])
    labels = rules.get("labels", {})
    if not isinstance(include, list) or not include or any(not isinstance(item, str) or not item for item in include):
        raise ValueError(f"{source}: release_assets.include must contain patterns")
    if not isinstance(exclude, list) or any(not isinstance(item, str) or not item for item in exclude):
        raise ValueError(f"{source}: release_assets.exclude must contain patterns")
    if not isinstance(labels, dict) or any(
        not isinstance(key, str) or not isinstance(value, str) or not value.strip()
        for key, value in labels.items()
    ):
        raise ValueError(f"{source}: release_assets.labels must map file names to labels")
    return include, exclude, {key: value.strip() for key, value in labels.items()}
...
def normalize_digest(value):
    if not isinstance(value, str) or not value:
        return None
    digest = value.split(":", 1)[1] if value.lower().startswith("sha256:") else value
    return digest.lower() if sha256_pattern.fullmatch(digest) else None


def collect_release_assets(repository, release, manifest, source):
    include, exclude, labels = normalize_asset_rules(manifest, source)
    assets = release.get("assets")
    if not isinstance(assets, list):
        return []
    result = []
    expected_prefix = f"/{repository}/releases/download/".lower()
    for asset in assets:
        ...
        if direct_extension(name) not in allowed_direct_extensions:
            continue
        if not matches_any(name, include) or matches_any(name, exclude):
            continue
        parsed = urlparse(url)
...
def mod_info_from_manifest(registration, manifest, source, repository, ref):
    manifest_id = require_text(manifest, "id", source)
    if manifest_id != registration["id"]:
        raise ValueError(f"{source}: id must be {registration['id']}")
    ...
    description = require_text(info, "description", source)
    homepage_url = info.get("homepage_url", f"https://github.com/{repository}")
    if homepage_url:
        homepage_url = require_https(homepage_url, "homepage_url", source)
    icon_url = ""
    explicit_icon = info.get("icon_url")
    icon_path = info.get("icon")
    if isinstance(explicit_icon, str) and explicit_icon.strip():
        icon_url = require_https(explicit_icon.strip(), "icon_url", source)
    elif isinstance(icon_path, str) and icon_path.strip():
        icon_url = raw_url(repository, ref, icon_path.strip())
    result = {
        "id": registration["id"],
        "name": name,
        "author": author,
        ...
        "tags": normalize_tags(info.get("tags", []), source),
    }
    if icon_url:
...
def load_github_mod(registration, source):
    repository = require_text(registration, "repository", source)
    if not repo_pattern.fullmatch(repository):
        raise ValueError(f"{source}: repository must be owner/name")
    metadata_path = registration.get("metadata_path", "levimod.json")
    if not isinstance(metadata_path, str) or not metadata_path.strip() or metadata_path.startswith("/") or ".." in Path(metadata_path).parts:
        raise ValueError(f"{source}: metadata_path must be a repository-relative path")
    metadata_path = metadata_path.strip()
    tag_prefix = registration.get("tag_prefix", "v")
    if not isinstance(tag_prefix, str):
        raise ValueError(f"{source}: tag_prefix must be text")
    include_prereleases = registration.get("include_prereleases", False)
    if not isinstance(include_prereleases, bool):
        raise ValueError(f"{source}: include_prereleases must be true or false")
    bootstrap_ref = registration.get("bootstrap_ref", "")
    if bootstrap_ref is None:
        bootstrap_ref = ""
    if not isinstance(bootstrap_ref, str):
        raise ValueError(f"{source}: bootstrap_ref must be text")
    max_releases = registration.get("max_releases", 20)
    if not isinstance(max_releases, int) or max_releases < 1 or max_releases > 100:
        raise ValueError(f"{source}: max_releases must be between 1 and 100")

    releases = request_json(github_release_url(repository, max_releases), use_token=True)
    if not isinstance(releases, list):
        raise ValueError(f"{source}: GitHub releases response was invalid")
    ...
    for release in releases:
        ...
        if not isinstance(tag, str):
            continue
        expected_version = version_from_tag(tag, tag_prefix)
        ...
        manifest_ref = tag
        try:
            manifest, manifest_source = load_manifest(repository, metadata_path, manifest_ref)
        except ResourceNotFound:
            if not newest_candidate or not bootstrap_ref:
                print(f"Warning: {repository} {tag} has no {metadata_path}; skipping release")
                continue
            manifest_ref = bootstrap_ref
            try:
                manifest, manifest_source = load_manifest(repository, metadata_path, manifest_ref)
            except ResourceNotFound:
                print(f"Warning: {repository} has no {metadata_path} at {tag} or {bootstrap_ref}; skipping release")
                continue

        manifest_version = require_text(manifest, "version", manifest_source)
        if manifest_version != expected_version:
            print(
                f"Warning: {manifest_source}: version {manifest_version} does not match tag {tag}; skipping release"
            )
            continue
        minecraft_versions = normalize_minecraft_versions(manifest.get("minecraft_versions"), manifest_source)
        assets = collect_release_assets(repository, release, manifest, manifest_source)
        if not assets:
            print(f"Warning: {repository} {tag} has no supported release assets; skipping release")
            continue
        published_at = release.get("published_at") or release.get("created_at")
        if not isinstance(published_at, str) or not published_at.strip():
            print(f"Warning: {repository} {tag} has no published timestamp; skipping release")
            continue
        release_entry = {
        ...
        if catalog_mod is None:
            catalog_mod = mod_info_from_manifest(
                registration, manifest, manifest_source, repository, manifest_ref
            )

    if catalog_mod is None or not catalog_releases:
        raise ValueError(f"{source}: no valid GitHub releases were discovered")
    catalog_releases.sort(key=lambda release: release["published_at"], reverse=True)
...
def load_registration(source, previous_mods):
    ...
    if not isinstance(data, dict):
        raise ValueError(f"{source}: mod.json must be an object")
    ...
    if not isinstance(enabled, bool):
        raise ValueError(f"{source}: enabled must be true or false")
    if not enabled:
        return None
    provider = data.get("provider", "static")
    if not isinstance(provider, str):
        raise ValueError(f"{source}: provider must be text")
    provider = provider.strip().lower()
    if provider == "static":
        return load_static_mod(data, source)
    if provider != "github":
        raise ValueError(f"{source}: provider must be static or github")
    try:
        ...
        if previous is None:
            raise
        print(f"Warning: {source}: {error}; keeping previous published entry")
        return previous
...
def main():
    ...
    mods = []
    for source in sorted(mods_root.glob("*/mod.json")):
        mod = load_registration(source, previous_mods)
        if mod is not None:
            mods.append(mod)
    ...
    (output / "catalog.json").write_text(
        json.dumps({"schema_version": 2, "mods": mods}, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    print(f"Generated catalog with {len(mods)} mods")
